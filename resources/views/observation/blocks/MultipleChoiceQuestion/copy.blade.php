@extends('layouts.blockCopyForm')

@section('block-copy-form')
<div class="card-content">
    <div class="card-title">
        Copy multiple choice question
    </div>
    <div class="row questionnaire-input-count">
        <div class="input-field col s12">
            <i class="material-icons prefix grey-text">short_text</i>
            <input type="text" id="question" name="question" value="{{ old('question') ? old('question') : $block->data['question'] }}">
        ...
        <div class="options-wrapper">
            ...
            <div class="option-wrapper">
                <div class="input-field col s8">
                    ...
                    <input type="text" id="option-{{ $i }}-text" name="option[{{ $i }}][text]" value="{{ old('option.'.$i.'.text') ? old('option.'.$i.'.text') : ( isset($block->data['options'][$i]['text']) ? $block->data['options'][$i]['text'] : "") }}">
                    <label for="option-{{ $i }}-text">Response option</label>
                </div>
                <div class="input-field col s4">
                    <i class="material-icons prefix grey-text">equalizer</i>
                    <input type="text" id="option-{{ $i }}-score" name="option[{{ $i }}][score]" value="{{ is_numeric(old('option.'.$i.'.score')) ? old('option.'.$i.'.score') : ( isset($block->data['options'][$i]['score']) ? $block->data['options'][$i]['score'] : "") }}">
...
<template id="template-question-option" style="display:none">
    <div class="option-wrapper">
        <div class="input-field col s8">
            <i class="material-icons prefix grey-text">check_box</i>
            <input type="text" id="option-{{ $i }}-text" name="option[__key__][text]" value="">
            <label for="option-{{ $i }}-text">Response option</label>
        </div>
        <div class="input-field col s4">
            <i class="material-icons prefix grey-text">equalizer</i>
            <input type="number" id="option-{{ $i }}-score" name="option[__key__][score]" value="">
            <label for="option-{{ $i }}-score">Score</label>
        </div>
    </div>
</template>
