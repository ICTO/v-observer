<div class="list-row-wrapper">
    ...
    <ul id='dropdown-block-{{ $block->id }}' class='dropdown-content action-btn'>
        <li><a href="{{ action('Observation\QuestionaireController@getEditBlock', $block->id ) }}">Edit</a></li>
        <li><a href="{{ action('Observation\QuestionaireController@getRemoveBlock', $block->id ) }}">Remove</a></li>
        @if( $block_types[$block->type]::canAddChildBlock() )
        <li class="divider"></li>
        <li><a href="#" class="black-text">Add child</a></li>
        @foreach($block_types as $type => $class)
        <li><a href="{{ action('Observation\QuestionaireController@getCreateBlock', [$questionaire->id, $type, $block->id]) }}" ><i class="material-icons left">add</i>{{ $class::getHumanName() }}</a></li>
        @endforeach
        @endif
    </ul>
    @endcan
